...
**Lightbox Panel Animation:**
- All lightbox panels (image frame, sidebar, nav toolbar) use a custom enter/exit animation: each panel expands from a thin blinking horizontal line into the full panel on open, and collapses back to a line on close.
- Timings live in `client/config.yaml` under `lightbox.panelAnimation` (`enterDurationMs`, `exitDurationMs`, `lineHoldMs`, `lineBlinkCount`, `lineBlinkIntervalMs`, `contentFadeInDelayMs`). Out-of-range or invalid values fall back to defaults with a `console.warn`; see `client/src/config/lightbox.ts`.
- In reduce-effects mode (in-app toggle or `prefers-reduced-motion`), the timings are ignored and the panels appear/disappear instantly.

...
// Test the single-pass image mapping logic directly.
//
// The gallery no longer preloads every image behind a placeholder before
// rendering. As soon as the image *list* arrives, useCampaignLoader builds the
// final ImageData[] in one pass (real src + loadedSrc, isLoading:false) and the
// browser streams thumbnails in natively via <img loading="lazy">. These tests
// mirror that mapping (see useCampaignLoader.loadCampaignImages).
describe('Campaign Image Mapping Logic', () => {
  // Mirrors the single-pass map in useCampaignLoader.loadCampaignImages.
  function mapImages(campaignImages, campaignId) {
    return campaignImages.map(img => ({
      ...img,
      originalSrc: img.src,
      loadedSrc: img.src,
      isLoading: false,
  ...
  it('builds ready entries with real src on the first pass (no null-src placeholder)', () => {
    const campaignImages = [
      { src: 'https://example.com/image1.jpg', fileName: 'image1.jpg' },
      { src: 'https://example.com/image2.jpg', fileName: 'image2.jpg' },
    ];

    const images = mapImages(campaignImages, 'ep1');

    expect(images).toHaveLength(2);
    images.forEach((img, index) => {
      expect(img.fileName).toBe(campaignImages[index].fileName);
      expect(img.src).toBe(campaignImages[index].src);
      expect(img.originalSrc).toBe(campaignImages[index].src);
      expect(img.loadedSrc).toBe(campaignImages[index].src);
      expect(img.isLoading).toBe(false);
    });
  });

  it('exposes the real CDN url immediately so the browser can lazy-load it', () => {
    const campaignImages = [
      { src: 'https://cdn.example.com/real-image.jpg', fileName: 'real-image.jpg' },
    ];

    const [img] = mapImages(campaignImages, 'ep1');

    // The src/loadedSrc the <img> renders is the real URL right away — there is
    // no intermediate null-src phase anymore.
    expect(img.src).toBe('https://cdn.example.com/real-image.jpg');
    expect(img.loadedSrc).toBe('https://cdn.example.com/real-image.jpg');
    expect(img.isLoading).toBe(false);
  });

  it('stamps the active campaign id onto every entry', () => {
    const campaignImages = [
    ...
    const images = mapImages(campaignImages, 'S04E11');

    images.forEach(img => {
      expect(img.campaignId).toBe('S04E11');
      expect(img.isLoading).toBe(false);
    });
  });

  it('preserves blobPath and author fields carried from the API response', () => {
    const campaignImages = [
      {
        src: 'https://example.com/img.jpg',
        fileName: 'img.jpg',
        blobPath: 'avatars/Season4/Ep/img.jpg',
        author: { handle: '@artist', name: 'Artist' },
      },
    ];

    const [img] = mapImages(campaignImages, 'ep1');

    expect(img.blobPath).toBe('avatars/Season4/Ep/img.jpg');
    expect(img.author).toEqual({ handle: '@artist', name: 'Artist' });
  });
});

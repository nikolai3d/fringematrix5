/**
 * PWA / service-worker configuration (vite-plugin-pwa, bead kwz9).
 *
 * The option objects live here as plain data so the PWA contract can be
 * unit-tested directly from the source of truth (see test/pwa.test.js)
 * without requiring a production build of dist/sw.js + manifest.webmanifest.
 *
 * vite.config.js imports `pwaOptions` and passes it straight to VitePWA().
 * The generated build is identical to inlining these objects.
 */
...
export const pwaWorkbox = {
  ...
  runtimeCaching: [
    ...
    },
    {
      ...
      //
      // CacheFirst is safe because blob images are immutable per URL —
      // replacements are uploaded to new paths (new URL) rather than
      // overwritten in place, so a stale cached entry is never wrong for a
      // given URL. The maintenance tooling assigns new paths/URLs instead
